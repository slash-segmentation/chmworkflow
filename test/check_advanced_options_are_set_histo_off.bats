...
#
#
@test "check advanced options are properly being passed histo false" {
  ...
  # Run kepler.sh with
  run $KEPLER_SH -runwf -redirectgui $THE_TMP -histoCheckBox false -overlap 50x50 -tilesPerTask 33 -tileSize 4x5 -trainedModel /bar -inputImages /images -CWS_user johnny -CWS_jobname foo -CWS_jobid 43 -createChmJob $THE_TMP/bin/createchm -CWS_outputdir $THE_TMP $CHM_WF
  ...
  [ "${lines[0]}" == "createpretrained $THE_TMP/run -m /bar -i /images -T 33 -b 4x5 -o 50x50 -h" ]
  ...
  [ "${lines[9]}" == "CHM options:  -T 33 -b 4x5 -o 50x50 -h" ]
